...
### v1.0.23 Integration Dependencies & Gates (append-only)

#### 1) Inventory truth for pricing governance
- The **stale-price queue must include only in-stock stale qualities**.  
- “In stock” is determined via CRM cache/mirrors fed by Contract v1.0.23 `stock.changed` events (not ad-hoc queries).

#### 2) Credit + payment + shipment gating intent (authoritative)
- Shipment approval must be blocked if:
  - `deal.credit_blocked = true` (until manager override clears it), OR
  - cash customer and payment not confirmed **per-PO**, OR
  - contract signature gate not satisfied for cash deals (signed_received or waived).
- Only Sales Manager (or explicit permission grants) can confirm payments and approve shipments.

#### 3) Email system constraints
- Customer status emails are CRM-owned, triggered by WMS lifecycle events, and must be idempotent.
- Shipped/Delivered defaults ON; Picking/Prepared defaults OFF unless explicitly enabled per org.
