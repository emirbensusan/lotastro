...
### v1.0.23 integration alignment notes (append-only)
This document remains the pricing thresholds input appendix. The following clarifications ensure it stays aligned with Contract v1.0.23 and the CRM-owned gating model:

1) **_toggle + default behavior**
- Customer-facing emails are conservative by default (“Defaults OFF” principle).
- WMS must not email customers; CRM owns customer status emails triggered by WMS lifecycle events.

2) **Idempotency requirement (extended)**
- Any approval action (discount approval, payment confirmation, shipment approval, credit override) and any outbound email must be idempotent so retries never duplicate side effects.

3) **Sequence handling (anti-regression)**
- If any business workflow relies on ordered updates (e.g., org access changes), the receiver must ignore out-of-order sequence updates and log them.

4) **stock.changed dependency**
- Pricing staleness checks that depend on “in stock” must use the CRM cache/mirror updated by `stock.changed` (Contract v1.0.23), so stale review queues include only in-stock items.
