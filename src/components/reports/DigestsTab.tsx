...
const DigestsTab: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      toast.error(String(t('digests.loadError')));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleDigest = async (digest: DigestConfig) => {
    try {
      ...
      toast.success(
        !digest.is_enabled 
          ? String(t('digests.enabled'))
          : String(t('digests.disabled'))
      );
    } catch (error) {
      console.error('Error toggling digest:', error);
      toast.error(String(t('digests.updateError')));
    }
  };
  ...
  const handleSaveDigest = async () => {
    ...
    try {
      ...
      toast.success(String(t('digests.saveSuccess')));
      setEditDialogOpen(false);
      fetchData();
    } catch (error) {
      console.error('Error saving digest:', error);
      toast.error(String(t('digests.saveError')));
    } finally {
      setSaving(false);
    }
  };
  ...
  const formatScheduleDescription = (digest: DigestConfig) => {
    ...
    switch (digest.schedule_type) {
      ...
      case 'quarterly':
        return `${t('quarterly')} ${config.day_of_month || 1}. ${t('day')} ${time} (${tz})`;
      default:
        return `${time} (${tz})`;
    }
  };
  ...
  const formatRecipients = (digest: DigestConfig) => {
    ...
    if (recipients.roles?.length) {
      parts.push(`${recipients.roles.length} ${t('digests.roles')}`);
    }
    if (recipients.emails?.length) {
      parts.push(`${recipients.emails.length} ${t('digests.emails')}`);
    }
    
    return parts.length > 0 ? parts.join(', ') : String(t('digests.noRecipients'));
  };
    ...
    <div className="space-y-6">
              ...
              <CardContent className="space-y-3">
                ...
                {digest.last_sent_at && (
                  <div className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-muted-foreground">
                      {t('digests.lastSent')}: {formatDistanceToNow(new Date(digest.last_sent_at), { addSuffix: true })}
                    </span>
                  </div>
                )}
                ...
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-muted-foreground">
                    {t('digests.cooldown')}: {digest.cooldown_hours}h
                  </span>
                </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base flex items-center gap-2">
            ...
            {t('digests.templateSchedules')}
          </CardTitle>
          <CardDescription>
            {t('digests.templateSchedulesDescription')}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {schedules.length === 0 ? (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {schedule.schedule_type} • {schedule.description || t('digests.noDescription')}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    {schedule.next_run_at && (
                      <span className="text-xs text-muted-foreground">
                        {t('digests.next')}: {format(new Date(schedule.next_run_at), 'MMM d, HH:mm')}
                      </span>
                    )}
                  </div>
                </div>
      ...
      <Dialog open={editDialogOpen} onOpenChange={setEditDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
          <DialogHeader>
            <DialogTitle>
              {editingDigest && getDigestTypeLabel(editingDigest.digest_type)} - {t('configuration')}
            </DialogTitle>
            <DialogDescription>
              {t('digests.editDescription')}
            </DialogDescription>
          </DialogHeader>
          ...
          <Tabs defaultValue="schedule" className="flex-1 overflow-hidden flex flex-col">
            ...
            <ScrollArea className="flex-1 mt-4">
              <TabsContent value="schedule" className="space-y-4 pr-4">
                ...
                {/* Cooldown */}
                <div className="space-y-2">
                  <Label>{t('digests.cooldownHours')}</Label>
                  <Input
                    type="number"
                    min={1}
                    max={168}
              ...
              <TabsContent value="recipients" className="space-y-4 pr-4">
                ...
                <div className="space-y-3">
                  <Label>{t('digests.roleBasedRecipients')}</Label>
                  <p className="text-xs text-muted-foreground">
                    {t('digests.roleBasedDescription')}
                  </p>
                ...
                <div className="space-y-3">
                  <Label>{t('digests.additionalEmails')}</Label>
                  <p className="text-xs text-muted-foreground">
                    {t('digests.additionalEmailsDescription')}
                  </p>
                ...
                <div className="bg-muted/50 rounded-lg p-4 mt-4">
                  <p className="text-sm font-medium mb-2">
                    {t('digests.recipientSummary')}
                  </p>
                  <div className="text-sm text-muted-foreground space-y-1">
                    <p>
                      {t('digests.rolesLabel')}:{' '}
                      {formData.roles.length > 0 
                        ? formData.roles.map(r => ROLES.find(role => role.value === r)?.label).join(', ')
                        : t('digests.noneSelected')}
                    </p>
                    <p>
                      {t('digests.emailsLabel')}:{' '}
                      {formData.emails.length > 0 
                        ? `${formData.emails.length} ${t('digests.addresses')}`
                        : t('digests.noneAdded')}
                    </p>
                  </div>
                </div>
