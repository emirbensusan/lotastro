        ...
        <div className="flex gap-4 items-center flex-wrap">
          ...
          {reportType !== 'sales' && (
            <div className="flex items-center space-x-2">
              ...
              />
              <Label htmlFor="in-stock-only">{t('inStockOnly')}</Label>
            </div>
          )}
        </div>
          ...
          {reportType === 'sales' && reportData.salesData ? (
                    ...
                    <div key={index} className="border rounded-lg p-4 space-y-2">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div>
                          ...
                          <p className="text-muted-foreground">
                            {sale.fulfilledAt ? new Date(sale.fulfilledAt).toLocaleDateString() : t('pending')}
                          </p>
                        </div>
                      </div>
                      
                      <div className="mt-2">
                        <span className="font-medium text-sm">{t('lots')}:</span>
                        <div className="grid gap-2 mt-1">
                          {sale.lots.map((lot, lotIndex) => (
                            <div key={lotIndex} className="flex items-center gap-4 text-xs bg-muted/50 p-2 rounded">
                              <span><strong>{t('quality')}:</strong> {lot.quality}</span>
                              <span><strong>{t('color')}:</strong> {lot.color}</span>
                              <span><strong>{t('lot')}:</strong> {lot.lotNumber}</span>
                              <span><strong>{t('rolls')}:</strong> {lot.rollCount}</span>
                              <span><strong>{t('meters')}:</strong> {lot.meters}</span>
                              {lot.ageDays && (
                              ...
                              {lot.lotEntryDate && (
                                <span><strong>{t('lotDate')}:</strong> {new Date(lot.lotEntryDate).toLocaleDateString()}</span>
                              )}
                            </div>
                          ))}
                        </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card>
                  <CardHeader>
                    <CardTitle>{t('lotsByQuality')}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-2">
                    {Object.entries(reportData.lotsByQuality).map(([quality, count]) => (
                      <div key={quality} className="flex justify-between items-center">
                        ...
                        <Badge variant="secondary">{count} {t('lots')}</Badge>
                      </div>
                    ))}
                  </CardContent>
                </Card>
